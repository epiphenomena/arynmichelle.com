...
// Define the singles data file path
$music_file = __DIR__ . '/../../../data/singles.json';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // 1. Handle Latest Single Updates
    ...
    // Latest Single Image Upload
    if (isset($_FILES['latest_image']) && $_FILES['latest_image']['error'] == 0) {
        $latest_image = handle_image_upload('latest_image', 'latest-single-cover', 'latest single cover');
        if ($latest_image) {
            $music_data['latest_album']['cover_image'] = '/' . $latest_image;
        } else {
            $error_msg = "Failed to upload latest single cover. Check file type and size.";
        }
    }

    // 2. Handle Previous Singles
    if (isset($_POST['delete_project'])) {
        ...
        $new_project = [
            'title' => 'New Single',
            'cover_image' => 'https://placehold.co/600x600/111/fff?text=New+Single',
            'link' => '#'
        ];
        ...
        foreach ($order as $original_index) {
            ...
            // Handle individual single image uploads
            $img_field = 'project_image_' . $original_index;
            $uploaded_img = handle_image_upload($img_field, 'single-cover-' . $original_index, 'single cover');
            if ($uploaded_img) {
    ...
    $music_data = json_decode(file_get_contents($music_file), true);
    $success_msg = !isset($error_msg) ? "Singles settings saved successfully!" : "";
}
...
<main>
    <div class="container">
        ...
        <p>Manage the singles layout, latest single, and previous singles.</p>

        <form method="post" enctype="multipart/form-data" id="main-form">
            <!-- Latest Single Hero Section -->
            <section class="admin-section">
                <h2>Latest Single (Hero Section)</h2>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-col">
                            <div class="form-group">
                                <label for="latest_title">Single Title</label>
            ...
            <section class="admin-section">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem;">
                    <h2>Singles Grid</h2>
                    <button type="submit" name="add_project" class="btn btn-secondary">Add New Single</button>
                </div>
                    ...
                    <div class="project-admin-item" data-original-index="<?php echo $index; ?>" draggable="true">
                        ...
                        <div class="project-admin-content">
                            <div class="form-row" style="align-items: center;">
                                ...
                                <div class="form-col">
                                    <input type="text" name="project_title[<?php echo $index; ?>]" value="<?php echo htmlspecialchars($project['title']); ?>" placeholder="Single Title">
                                    <input type="text" name="project_link[<?php echo $index; ?>]" value="<?php echo htmlspecialchars($project['link']); ?>" placeholder="Link (Spotify, Apple Music, etc)">
                                </div>
                                <div class="form-col" style="flex: 0 0 50px;">
                                    <button type="submit" name="delete_project" value="<?php echo $index; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Remove this single?')">×</button>
            ...
            <div style="margin-top: 2rem; border-top: 1px solid #ddd; padding-top: 2rem; text-align: right;">
                <button type="submit" class="btn btn-primary btn-lg">Save All Singles Settings</button>
            </div>
        </form>
    </div>
</main>
